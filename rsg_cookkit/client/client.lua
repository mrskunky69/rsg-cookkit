...
local djdecksprop = {
    ...
    exports['rsg-target']:AddTargetModel(djdecksprop, {
        options = {
            {
				type = "client",
				event = "rsg_cookkit:client:pickupDJEquipment",
                icon = "fas fa-undo",
...
-- place dj equipment
RegisterNetEvent('rsg_cookkit:client:placeDJEquipment', function()
    ...
    local object = CreateObject(GetHashKey('p_campfirecombined01x'), x, y, z, true, false, false)
...
RegisterNetEvent('rsg_cookkit:client:pickupDJEquipment', function()
    ...
    if not DoesEntityExist(obj) then
        TriggerServerEvent('rsg_cookkit:server:pickedup', deployeddecks)
        TriggerServerEvent('rsg_cookkit:server:pickeupdecks')
        deployeddecks = nil
    end
    Wait(500)
    ClearPedTasks(PlayerPedId())
